...
Shows what the running session has delegated: the Claude session on one row, with its subagents and Codex CLI runs listed underneath, indented under a left rule.

| Element | Description |
|---------|-------------|
| Session row | Blue pulsing dot = thinking, yellow = waiting, gray = idle. Shows the model, how many children are running out of the total, and context usage. Click to focus the session |
| Subagent row | Blue pulsing dot while active. Shows the agent's name, its own model, and time since last activity; the task description appears in the tooltip |
| Codex row | Green badge while running, with the subcommand, model, and elapsed time. Click to open the rollout transcript |

Running children are listed first, then completed ones by most recent activity. Completed children stay listed rather than disappearing, capped at 8 rows in the sidebar and 30 in the editor view, with a `+N more` row for the remainder.

Codex CLI sessions are detected from rollout files under `~/.codex/sessions` (or `CODEX_HOME`) and matched to Claude sessions by working directory. The sidebar shows the focused or most recently active session; the title bar icon opens a full-screen editor view that lays all active sessions out in columns. Selecting a session in the Sessions panel focuses it here.

> This extension is not affiliated with, endorsed by, or sponsored by OpenAI. It only reads local log files generated by your own Codex CLI usage; no data is sent to any external service.
...
| Model Fable | Fable sessions only |
...
### Pricing (per 1M tokens, as of 2026-07-25)
...
| `defaultGroup` | `none` | Default grouping mode (none/project/status/custom) |
...
| `debugGraphStateFile` | (empty) | Debug only: dump the Agent Graph input state to this JSON file on each refresh. User/machine scope only — it cannot be set per workspace, and the target file is overwritten |

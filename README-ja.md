...
実行中のセッションをノードグラフでリアルタイム可視化。Claudeセッションを親ノード、起動されたサブエージェントとCodex CLI実行を子ノードとして表示。

| 要素 | 説明 |
|------|------|
| Claudeノード | 青の点滅=thinking、黄=waiting、灰=idle。クリックでセッションにフォーカス |
| サブエージェントノード | 実行中は青の点滅。ツールチップにタスク内容を表示。完了分は `N done` 件数に集約 |
| Codexノード | 実行中は緑、完了後5分間表示。クリックでrolloutトランスクリプトを開く |

Codex CLIセッションは `~/.codex/sessions`（または `CODEX_HOME`）のrolloutファイルから検出し、作業ディレクトリでClaudeセッションと対応付けます。サイドバーはフォーカス中または直近アクティブな1セッションを表示し、タイトルバーのアイコンから全アクティブセッションのフルスクリーン表示をエディタで開けます。Sessionsパネルでセッションを選択すると対応するグラフにフォーカスします。
